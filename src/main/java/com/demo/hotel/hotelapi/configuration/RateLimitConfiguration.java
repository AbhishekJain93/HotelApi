...
public class RateLimitConfiguration {
		...
		synchronized (lock) {
			...
			if (rateLimitModel.nextFrame.before(currentTimestamp)) {
				...
				if (rateLimitModel.currentCounter + 1 <= rateLimitModel.counter) {
					...
					rateLimitModel.counter = getMaxCounter(apiName, defaultRate);
					rateLimitModel.nextFrame = new Timestamp(
							System.currentTimeMillis() + suspensionMinutes * 60 * 1000);
	...
	private int getMaxCounter(String apiName, String defaultRate) {
	...
	private Timestamp getNextTimeFrame(String apiName, String defaultRate) {
